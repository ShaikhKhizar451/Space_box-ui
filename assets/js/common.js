...
async function getProfileInfo() {
    ...
    if (id == null) {
        ...
        showloader();
        let response = await fetch('http://127.0.0.1:8000/getUserWithId/' + id, {
            headers: myHeaders,
            method: 'Get'
        });
        let result = await response.json();
...
async function getSuggestionUser() {
    let response = await fetch('http://127.0.0.1:8000/suggestionUser', {
        headers: myHeaders,
        method: 'Get'
    });
    let result = await response.json();
...
async function followFunction(data) {
    id = data.id;
    let response = await fetch('http://127.0.0.1:8000/follow/' + id, {
        headers: myHeaders,
        method: 'Put'
    });
    let result = await response.json();
